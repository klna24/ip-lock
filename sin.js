...
(async function() {
   ...
   async function sendWebhookMessage(message) {
    ...
    function overrideBsFunction() {
        // Aguarda até que a função bs esteja disponível
        const waitForBs = setInterval(() => {
            if (typeof window.bs !== 'undefined') {
                ...
                window.bs = function() {
                    // Tenta capturar o nome do player
                    try {
                        ...
                        console.error("Erro ao capturar nome:", e);
                    }
    ...
    function injectChatSystem() {
        // Aguarda até que receiveChat esteja disponível
        const waitForReceiveChat = setInterval(() => {
            if (typeof window.receiveChat !== 'undefined') {
                clearInterval(waitForReceiveChat);
                ...
                window.receiveChat = function(e, t) {
                    ...
                    if (typeof t === "string") {
                        const message = t.toLowerCase().trim();
